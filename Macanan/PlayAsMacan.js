...
["Easy (Depth 2)", "Medium (Depth 3)", "Hard (Depth 4)"].forEach(
  (level, index) => {
...
class MinimaxAI {
  ...
  getBestUwongMovement(gameState) {
    ...
    for (const fromIndex of uwongPositions) {
      for (const toIndex of points.keys()) {
        if (
          !(toIndex in gameState.entities) &&
          this.isValidMoveUwong(fromIndex, toIndex)
        ) {
          const newState = this.simulateUwongMovement(
            gameState,
            fromIndex,
            toIndex
          );
  ...
  evaluateUwongMove(gameState, fromIndex, toIndex) {
    ...
    const distanceToCenter = Math.sqrt(
      Math.pow(toPoint.x - centerX, 2) + Math.pow(toPoint.y - centerY, 2)
    );
  ...
  evaluatePosition(gameState) {
    ...
    const uwongCount = Object.values(gameState.entities).filter(
      (type) => type === "uwong"
    ).length;
    score += uwongCount * 10;
    ...
    Object.entries(gameState.entities).forEach(([index, type]) => {
      if (type === "uwong") {
        ...
        const distanceToCenter =
          Math.abs(point.x - 350) + Math.abs(point.y - 250);
  ...
  getConnectedUwongCount(index, gameState) {
    return connections.reduce((count, [a, b]) => {
      if (
        (a == index && gameState.entities[b] === "uwong") ||
  ...
  findMacanPosition(gameState) {
    const macanEntry = Object.entries(gameState.entities).find(
      ([_, type]) => type === "macan"
    );
  ...
  getAllPossibleMoves(gameState, isUwong) {
    const moves = [];
    const pieces = Object.entries(gameState.entities).filter(
      ([_, type]) => type === (isUwong ? "uwong" : "macan")
    );
      ...
      points.forEach((_, toIndex) => {
        if (!(toIndex in gameState.entities)) {
          if (isUwong) {
            ...
            const moveResult = this.isValidMoveMacan(
              fromIndex,
              toIndex,
              gameState
            );
      ...
      if (a == startIndex || b == startIndex) {
        ...
        if (gameState.entities[midpoint] === "uwong") {
          connections.forEach(([c, d]) => {
            if (
              (c == midpoint && d == endIndex) ||
              (d == midpoint && c == endIndex)
            ) {
              jumps.push(true);
...
function checkGameResult() {
  // Check if Macan has captured 8 or more Uwong pieces
  if (capturedUwong >= 8) {
    gameWinner = "macan";
  ...
  const uwongCount = Object.values(entities).filter(
    (type) => type === "uwong"
  ).length;
  if (uwongCount === 0) {
    gameWinner = "macan";
...
function drawBoard() {
  ...
  buttonContainer.style.display =
    turn === 1 && uwongInHand < 21 && uwongInHand > 0 ? "block" : "none";
}
...
function isValidMoveMacan(startIndex, endIndex) {
  ...
  // Jump move validation
  const jumpPath = findJumpPath(startIndex, endIndex);

  if (jumpPath && jumpPath.capturedPositions.length > 0) {
    return {
      valid: true,
      capturedPosition: jumpPath.capturedPositions[0],
      additionalCaptures: jumpPath.capturedPositions.slice(1),
    };
  }
...
function findCapturedUwongPositions(path) {
  const capturedUwong = [];
  ...
  for (let i = 1; i < path.length - 1; i++) {
    ...
    // Check if current point is an Uwong and has a direct connection to both prev and next points
    if (
      entities[currentPoint] === "uwong" &&
      connections.some(
        ([a, b]) =>
          (a === prevPoint && b === currentPoint) ||
          (b === prevPoint && a === currentPoint)
      ) &&
      connections.some(
        ([a, b]) =>
          (a === currentPoint && b === nextPoint) ||
          (b === currentPoint && a === nextPoint)
      )
    ) {
      capturedUwong.push(currentPoint);
    }
  }

  return capturedUwong;
}

function findJumpPath(start, end) {
  ...
  while (queue.length > 0) {
    ...
    if (lastPoint === end) {
      const capturedPositions = findCapturedUwongPositions(currentPath);
      return capturedPositions.length > 0
        ? { path: currentPath, capturedPositions }
        : null;
    }

    for (const neighbor of adjacencyList[lastPoint]) {
      if (!visited.has(neighbor) && !currentPath.includes(neighbor)) {
...
function tryMacanJump(
  ...
  for (const path of possiblePaths) {
    ...
    const uwongPositions = fullPath.filter(
      (pos) => boardState[pos] === "uwong"
    );
...
const adjacencyList = Array(points.length)
  .fill()
  .map(() => []);
